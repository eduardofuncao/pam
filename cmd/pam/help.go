...
func (a *App) PrintGeneralHelp() {
	...
	fmt.Println(styles.Faint.Render("Save, edit, and run named SQL queries across connections."))
	fmt.Println()
	...
	fmt.Println("  init        " + styles.Faint.Render("Create and Save a new database connection"))
	fmt.Println("  switch      " + styles.Faint.Render("Switch the active connection (alias: use)"))
	fmt.Println("  add         " + styles.Faint.Render("Save a new named query (alias: save)"))
	...
	fmt.Println("  help        " + styles.Faint.Render("Show help for pam or a specific command"))
	fmt.Println()

	// Short help
	fmt.Println(styles.Title.Render("Help"))
	fmt.Println("  pam help              " + styles.Faint.Render("Show this help"))
	fmt.Println("  pam help <command>    " + styles.Faint.Render("Show detailed help for a specific command"))
	fmt.Println()
...
func (a *App) PrintCommandHelp() {
	...
	case "switch", "use":
		...
		fmt.Println(styles.Faint.Render("Switch the active connection used by other commands."))
		fmt.Println()
		section("Usage")
		fmt.Println("  pam switch/use <connection-name>")
		fmt.Println()
		section("Description")
		fmt.Println("  - Sets the connection to be used by 'add', 'run', 'list queries', etc.")
		fmt.Println()
		section("Examples")
		fmt.Println("  pam switch dev")
	...
	case "add", "save":
		section("Command: add")
		fmt.Println(styles.Faint.Render("Save a new named query under the current connection."))
		fmt.Println()
		section("Usage")
		fmt.Println("  pam add <query-name> [query]")
		fmt.Println()
		section("Description")
		fmt.Println("  - If [query] is omitted, pam opens $EDITOR (default: vim) so you")
	...
	case "run", "query":
		section("Command: run")
		fmt.Println(styles.Faint.Render("Execute a saved query against the current connection and show results in an interactive table view."))
		fmt.Println()
		section("Usage")
		fmt.Println("  pam run <query-name-or-id> [--edit | -e]")
		fmt.Println("  pam query <query-name-or-id> [--edit | -e]")
		fmt.Println("  pam run                      " + styles.Faint.Render("# runs the last query for the active connection"))
		fmt.Println()
		section("Description")
		fmt.Println("  - Looks up a saved query by name or numeric ID and runs it against")
		fmt.Println("    the current connection.")
		fmt.Println("  - If no selector is provided, pam will try to execute the last query")
		...
		fmt.Println(styles.Faint.Render("When results are shown, you can interact with the table using the keyboard:"))
		fmt.Println()
		...
		fmt.Println(styles.Faint.Render("but the basic navigation, search/filtering, and quit commands are available."))
		fmt.Println()
		section("Examples")
		fmt.Println("  pam run list_users")
		fmt.Println("  pam run 2 --edit")
		fmt.Println("  pam query list_users")
		fmt.Println("  pam run              " + styles.Faint.Render("# run the last query for this connection"))

	case "list":
		section("Command: list")
		fmt.Println(styles.Faint.Render("List connections or queries. Defaults to queries."))
		fmt.Println()
		section("Usage")
		fmt.Println("  pam list [connections | queries] [search-term]")
		fmt.Println()
		section("Description")
		fmt.Println("  connections  " + styles.Faint.Render("List all configured connections; active one is highlighted."))
		fmt.Println("  queries      " + styles.Faint.Render("List all saved queries for the current connection, with SQL."))
		fmt.Println("               " + styles.Faint.Render("Optionally filter by search term (searches name and SQL)."))
		fmt.Println()
		section("Examples")
		fmt.Println("  pam list                      # lists queries for the current connection")
		fmt.Println("  pam list queries")
	...
	case "edit":
		...
		fmt.Println()
		section("Description")
		fmt.Println("  config   " + styles.Faint.Render("Edit the main configuration file (connections etc.)."))
		fmt.Println("  queries  " + styles.Faint.Render("Edit all queries for the current connection in one file."))
		fmt.Println()
		section("Examples")
	...
	case "history":
		...
		fmt.Println(styles.Faint.Render("Show query execution history (placeholder)."))
		fmt.Println()
		section("Usage")
		fmt.Println("  pam history")
		fmt.Println()
		section("Description")
		fmt.Println("  This command is currently a placeholder and will be implemented")
		fmt.Println("  in a future release.")

	case "help":
		section("Command: help")
		fmt.Println(styles.Faint.Render("Show general help or detailed help for a specific command."))
		fmt.Println()
		section("Usage")
